...
#         return redirect('admin_panel')  # refresh page

#     admins = user.objects.filter(role="admin")
#     return render(request, "principal.html", {"admins": admins})
...
def add_teacher(request):
    if request.method == "POST":
        ...
        user.objects.create(
            name=name,
            email=email,
            password="default123",   # required field
            role="teacher",
            subject=subject
        )
        return redirect(request.META.get("HTTP_REFERER"))
...
def edit_user(request, id):
    ...
    if request.method == "POST":
        ...
        if user.role == "student":
            user.class_name = request.POST.get("class")
