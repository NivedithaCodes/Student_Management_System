...
# views_option1.py  (copy into your app's views.py or merge)
from django.shortcuts import render, redirect, get_object_or_404
from django.contrib import messages
from .models import user, ClassRoom, Subject  # uses your existing lowercase 'user' model
# If you added AssignSubject, Syllabus, Timetable models to models.py, import them:
try:
    from .models import AssignSubject, Syllabus, Timetable
except Exception:
    AssignSubject = None
    Syllabus = None
    Timetable = None

# ---------- existing auth / pages (kept as you had) ----------
def home_view(request):
    return render(request, "home.html")
...
def login_view(request, popup=None):
    if request.method == "POST":
        ...
        try:
            user_obj = user.objects.get(email=email)
            if user_obj.password == password:
                # keep same behavior: redirect by role
                messages.success(request, "Login Successfully")
                if user_obj.role == "student":
                    return redirect('student')
                elif user_obj.role == "parent":
...
# Update principal_view to include classes & subjects (important)
def principal_view(request):
    # Render principal.html with admin/teacher/student lists + classes + subjects
    users_admin = user.objects.filter(role="admin")
    users_teacher = user.objects.filter(role="teacher")
    users_student = user.objects.filter(role="student")
    classes = ClassRoom.objects.all()
    subjects = Subject.objects.all()
    # include optional AssignSubject/Syllabus/Timetable if exist
    assigns = AssignSubject.objects.all() if AssignSubject else []
    syllabi = Syllabus.objects.all() if Syllabus else []
    timetables = Timetable.objects.all() if Timetable else []
...
# ---------- CRUD actions for admin dashboard (existing) ----------
def add_admin(request):
    if request.method == "POST":
        name = request.POST.get("name")
        email = request.POST.get("email")
...
# ---------- Class CRUD (already similar to what you had) ----------
def add_class(request):
    if request.method == "POST":
        class_name = request.POST.get("class_name")
        section = request.POST.get("section")
    ...
    # GET — show add form (if you have separate template)
    teachers = user.objects.filter(role='teacher')
    return render(request, 'add_class.html', {'teachers': teachers})
...
def add_subject(request):
    if request.method == "POST":
        # form fields that match your template
        subject_name = request.POST.get("subject_name") or request.POST.get("name")
        class_room_id = request.POST.get("class_room") or request.POST.get("class_id")
        teacher_id = request.POST.get("teacher") or None

        Subject.objects.create(
            subject_name=subject_name,
            assigned_class_id=class_room_id,
            teacher_id=teacher_id
        )
        return redirect('principal')
...
def edit_subject(request, id):
    subject = get_object_or_404(Subject, id=id)
    if request.method == "POST":
        subject.subject_name = request.POST.get("subject_name") or request.POST.get("name")
        subject.assigned_class_id = request.POST.get("class_room") or request.POST.get("class_id")
        subject.teacher_id = request.POST.get("teacher") or None
        subject.save()
        return redirect('principal')
...
# ---------- Assign Subject (teacher->subject->class) ----------
def assign_subject(request):
    # this view expects AssignSubject model exists
    if AssignSubject is None:
        messages.error(request, "AssignSubject model not found. Add it to models.py.")
        return redirect('principal')
    ...
    if request.method == "POST":
        ...
        AssignSubject.objects.create(teacher_id=teacher_id, class_room_id=class_id, subject_id=subject_id)
        return redirect('principal')

    teachers = user.objects.filter(role='teacher')
    classes = ClassRoom.objects.all()
    subjects = Subject.objects.all()
    return render(request, 'assign_subject.html', {'teachers': teachers, 'classes': classes, 'subjects': subjects})


def delete_assign_subject(request, id):
    if AssignSubject is None:
        messages.error(request, "AssignSubject model not found.")
        return redirect('principal')
    asg = get_object_or_404(AssignSubject, id=id)
    asg.delete()
    return redirect('principal')
...
def upload_syllabus(request):
    if Syllabus is None:
        messages.error(request, "Syllabus model not present.")
        return redirect('principal')
    ...
    if request.method == "POST":
        ...
        if file:
            Syllabus.objects.create(class_room_id=class_id, subject_id=subject_id, syllabus_file=file)
            return redirect('principal')
    classes = ClassRoom.objects.all()
    subjects = Subject.objects.all()
    return render(request, 'upload_syllabus.html', {'classes': classes, 'subjects': subjects})
...
def upload_timetable(request):
    if Timetable is None:
        messages.error(request, "Timetable model not present.")
        return redirect('principal')
    ...
    if request.method == "POST":
        ...
        if file:
            Timetable.objects.create(class_room_id=class_id, timetable_file=file)
            return redirect('principal')

    classes = ClassRoom.objects.all()
    return render(request, 'upload_timetable.html', {'classes': classes})
...
# ---------- Change Password (keeps plain password style to match your current system) ----------
def change_password(request, id):
    u = get_object_or_404(user, id=id)
    if request.method == "POST":
        old_pwd = request.POST.get("old_password")
