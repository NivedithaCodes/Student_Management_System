...
def add_class(request):
    if request.method == "POST":
        ...
        ClassRoom.objects.create(class_name=class_name, section=section,max_strength=max_strength)
...
def edit_class(request, id):
    ...
    if request.method == "POST":
        ...
        cls.teacher_id = request.POST.get("class_teacher")
        if cls.teacher_id:
            try:
                cls.class_teacher = user.objects.get(id=clsteacher_id)
            except user.DoesNotExist:
                cls.class_teacher = None
        cls.max_strength = request.POST.get("max_strength") or None
        cls.save()
        return redirect("/principal/?tab=class")   # NEW
