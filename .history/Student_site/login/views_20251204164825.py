...
def edit_class(request, id):
    ...
    return render(request, "edit_class.html", {"cls": cls,"users_teacher": users_teacher})
...
def edit_subject_base(request, id):
    ...
    return render(request, "edit_subject_base.html", {"sub": sub})
...
#         # You can save this to DB or send to admin
#         messages.success(request, "Notification sent to admin!")
#         return redirect("/teacher/?tab=notifications")
...
#         role="student",
#         assigned_class__in=[sub.assigned_class for sub in assigned_subjects]
#     )
...
#     # Students under teacher's assigned classes
#     students = user.objects.filter(
#         role="student",
#         class_name__in=[sub.assigned_class.class_name for sub in assigned_subjects]
#     )

#     # Timetable (teacher doesn't have teacher field, use class_room)
#     timetables = Timetable.objects.filter(
#         class_room__in=[sub.assigned_class for sub in assigned_subjects]
#     )
...
#     # Students from teacher's assigned classes
#     students = user.objects.filter(
#         role="student",
#         student_class__in=[sub.assigned_class for sub in assigned_subjects]
#     )

#     # Timetable for those classes
#     timetables = Timetable.objects.filter(
#         class_room__in=[sub.assigned_class for sub in assigned_subjects]
#     )
...
#     try:
#         # 3️⃣ Get the logged-in teacher
#         teacher = user.objects.get(id=request.session["user_id"])

#         # 4️⃣ Subjects assigned to the teacher
#         assigned_subjects = AssignSubject.objects.filter(teacher=teacher)

#         # 5️⃣ Classes assigned to this teacher
#         assigned_classes = [sub.assigned_class for sub in assigned_subjects]

#         # 6️⃣ Students in these classes
#         students = user.objects.filter(
#             role="student",
#             class_name__in=[cls.class_name for cls in assigned_classes]
#         )

#         # 7️⃣ Timetables for assigned classes
#         timetables = Timetable.objects.filter(class_room__in=assigned_classes)

#         # 8️⃣ Notifications sent by this teacher to admin
#         notifications = Notification.objects.filter(sender=teacher, is_for_admin=True)
...
#     except user.DoesNotExist:
#         # If user_id in session is invalid
#         return redirect("login")
...
#     # Students from classes assigned to teacher
#     class_names = [sub.assigned_class.class_name for sub in assigned_subjects if sub.assigned_class]
#     students = user.objects.filter(role="student", class_name__in=class_names)

#     # Timetables
#     timetables = Timetable.objects.filter(class_room__in=[sub.assigned_class for sub in assigned_subjects if sub.assigned_class])

#     # Notifications
#     notifications = Notification.objects.filter(sender=teacher).order_by('-created_at')
...
from .models import user, ClassRoom, SubjectBase, AssignedSubject, Syllabus, Timetable

def teacher_view(request):
    # Logged-in teacher
    teacher = request.user

    # Get tab from URL -> ?tab=student, ?tab=subject etc
    tab = request.GET.get("tab", "dashboard")
    ...
    classes = ClassRoom.objects.all()
    subjects_list = SubjectBase.objects.all()
    subjects = AssignedSubject.objects.all()
    syllabi = Syllabus.objects.all()
    timetables = Timetable.objects.all()
