...
#         subject__in=[s.subject for s in assigned_subjects]
#     )

#     # 5. Timetables only for assigned classes
...
def teacher_view(request):
    ...
    # 2. Subjects assigned to this teacher
    subjects = AssignSubject.objects.filter(teacher=teacher)

    # 3. Students only from assigned classes
    students = user.objects.filter(role='student', class_name__in=classes)

    # 4. Syllabus only for teacher's subjects & classes
    syllabi = Syllabus.objects.filter(subject__in=[s.subject for s in subjects],
                                       class_name__in=classes)

    # 5. Timetables only for assigned classes
    timetables = Timetable.objects.filter(class_name__in=classes)

    # Other users (teachers/admins)
    teachers = user.objects.filter(role='teacher')
    admins = user.objects.filter(role='admin')
    subjects_list = Subject.objects.all()  # Optional: all subjects for selection
