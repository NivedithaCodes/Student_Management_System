...
#             login(request, user_auth)   #  LOGIN HAPPENS HERE
#             messages.success(request, "Login Successfully")
...
def principal_view(request):
    ...
    total_assign_subjects = AssignSubject.objects.count()
    ...
    context = {
        ...
        # Add totals for dashboard cards
        "total_students": total_students,
        "total_teachers": total_teachers,
        "total_admins": total_admins,
        "total_classes": total_classes,
        "total_subjects": total_subjects,
        "total_assign_subjects": total_assign_subjects,
...
#     # All users (optional for admin view)
#     teachers = user.objects.filter(role='teacher')
#     admins = user.objects.filter(role='admin')

#     # 1. Assigned classes
#     assigned_classes = ClassRoom.objects.filter(class_teacher=teacher)

#     # 2. Assigned subjects
#     assigned_subjects = AssignSubject.objects.filter(teacher=teacher)

#     # 3. Students in assigned classes
#     students = user.objects.filter(role='student', class_name__in=assigned_classes)

#     # 4. Syllabi for assigned subjects and classes
#     syllabi = Syllabus.objects.filter(subject__in=[s.subject for s in assigned_subjects],
#                                        class_name__in=assigned_classes)

#     # 5. Timetables for assigned classes
#     timetables = Timetable.objects.filter(class_name__in=assigned_classes)

#     # Optional: list of all subjects for selection
#     subjects_list = Subject.objects.all()
...
#     # 1. Subjects assigned to this teacher
#     assigned_subjects = AssignSubject.objects.filter(teacher=teacher)

#     # 2. Classes assigned to this teacher (via assigned_subjects)
#     assigned_classes = ClassRoom.objects.filter(id__in=assigned_subjects.values_list('assigned_class', flat=True))

#     # 3. Students in the assigned classes
#     students = user.objects.filter(role='student', class_name__in=assigned_classes)

#     # 4. Syllabus only for teacher's subjects & classes
...
#     # Other users (for lists in template if needed)
#     teachers = user.objects.filter(role='teacher')
#     admins = user.objects.filter(role='admin')
...
def teacher_view(request):
    ...
    # 6. Students in assigned classes (use class_name string)
    students = user.objects.filter(
        role='student',
        class_name__in=assigned_classes.values_list("class_name", flat=True)
    )

    # 7. Syllabus (FIXED class_room instead of class_name)
    syllabi = Syllabus.objects.filter(
        subject__in=assigned_subjects.values_list('subject', flat=True),
        class_room__in=assigned_classes   
    )

    # 8. Timetable (same issue, FIXED)
