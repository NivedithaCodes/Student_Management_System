...
def teacher_view(request):
    ...
    # Classes related to the assigned subjects
    assigned_classes = ClassRoom.objects.filter(
        id__in=assigned_subjects.values_list('assigned_class', flat=True)
    )

    # Prepare structured data per class
    classes_info = []

    for cls in assigned_classes:
        # 1. Class Teacher students
        class_students = user.objects.filter(role='student', class_name=cls)

        # 2. Subject students (students taking subjects assigned to this teacher in this class)
        subject_ids = assigned_subjects.filter(assigned_class=cls).values_list('id', flat=True)
        subject_students = user.objects.filter(
            role='student',
            class_name=cls,
            subjects_assigned__id__in=subject_ids
        ).distinct()
    ...
    # Syllabus and timetable (optional, same as before)
    syllabi = Syllabus.objects.filter(
        subject__in=assigned_subjects.values_list('subject', flat=True),
        class_room__in=assigned_classes   
    )
    ...
    return render(request, "TeacherPages/teacher_courses.html", context)
