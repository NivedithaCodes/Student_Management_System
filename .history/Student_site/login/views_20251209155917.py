...
def teacher_view(request):
    ...
    # Classes teacher is assigned via subjects
    ...
    class_teacher_classes = ClassRoom.objects.filter(class_teacher=teacher)

    # Students grouped by class
    students_by_class = {}
    for cls in ClassRoom.objects.all():
        students_by_class[cls.id] = list(user.objects.filter(role='student', class_name=cls).values('id', 'name', 'roll_no'))

    # Subjects JSON for JS
    subjects_json = []
    for sub in AssignSubject.objects.all():
        subjects_json.append({
            "id": sub.id,
            "name": sub.subject.subject_name if sub.subject else "N/A",
            "class_id": sub.assigned_class.id if sub.assigned_class else None,
            "teacher_id": sub.teacher.id if sub.teacher else None
        })
    ...
    context = {
        ...
        "students_by_class": json.dumps(students_by_class, cls=DjangoJSONEncoder),
        "subjects_json": json.dumps(subjects_json),
        "teacher": teacher,
