...
from .models import user, ClassRoom, AssignSubject, Syllabus, Timetable
from django.utils import timezone
import json
...
def teacher_view(request):
    ...
    # 5. Classes assigned to this teacher (from subjects)
    assigned_classes = ClassRoom.objects.filter(
        id__in=assigned_subjects.values_list('assigned_class', flat=True)
    )

    # 6. Prepare students grouped by class
    students_by_class = {}
    for cls in assigned_classes:
