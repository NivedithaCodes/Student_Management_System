...
def teacher_view(request):
    ...
    # Create a simple list of IDs for template usage
    class_teacher_ids = list(class_teacher_classes.values_list('id', flat=True))


    # Subjects assigned to this teacher (as subject teacher)
    assigned_subjects = AssignSubject.objects.filter(teacher=logged_in_user)

    # All classes relevant for this teacher (union of class_teacher_classes + classes from assigned_subjects)
    ...
    # Determine which subjects a teacher can mark
    markable_subject_map = {}  # {class_id: [subject_ids teacher can mark]}
    for cls in relevant_classes:
        markable_subjects = []
        # class teacher can mark if subject has a subject teacher assigned
    ...
    # 1. students_by_class
    # 2. subject_students_map
    # 3. markable_subject_map
    # 4. relevant_classes
    # 5. assigned_subjects
...
def submit_attendance(request):
    ...
    # Permission check
    # Teacher can mark if:
    # 1. They are class teacher of the class AND assign_obj.teacher exists (subject teacher)
    # 2. They are the subject teacher for the assign_obj
    ...
    # Remove existing attendance for this class + subject + date
