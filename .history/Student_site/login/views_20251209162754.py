...
from django.shortcuts import render, redirect, get_object_or_404
from django.utils import timezone
from .models import user, ClassRoom, AssignSubject, Attendance
...
def teacher_view(request):
    ...
    # 2️⃣ Classes where teacher is class teacher
    class_teacher_classes = ClassRoom.objects.filter(class_teacher=logged_in_user)
    class_teacher_ids = list(class_teacher_classes.values_list('id', flat=True))  # for template JS

    # 3️⃣ Subjects assigned to this teacher (as subject teacher)
    assigned_subjects = AssignSubject.objects.filter(teacher=logged_in_user)

    # 4️⃣ Classes relevant for this teacher (union of class_teacher_classes + classes from assigned subjects)
    ...
    # 5️⃣ Build students by class
    students_by_class = {}
    for cls in relevant_classes:
        students_by_class[cls.id] = list(user.objects.filter(role='student', class_name=cls).order_by('name'))

    # 6️⃣ Map: subject -> students (only students in that class)
    subject_students_map = {}
    for assign in assigned_subjects:
        subject_students_map[assign.id] = list(
            user.objects.filter(role='student', class_name=assign.assigned_class).values_list('id', flat=True)
        )

    # 7️⃣ Determine which subjects a teacher can mark per class
    markable_subject_map = {}  # {class_id: [subject_ids teacher can mark]}
    for cls in relevant_classes:
        markable_subjects = []
        subjects_in_class = AssignSubject.objects.filter(assigned_class=cls)
        for subj in subjects_in_class:
            # Class teacher can mark if a subject teacher exists, or teacher is subject teacher
            if subj.teacher == logged_in_user or (cls.class_teacher == logged_in_user and subj.teacher):
                markable_subjects.append(subj.id)
        markable_subject_map[cls.id] = markable_subjects

    # 8️⃣ Build context for template
    context = {
        "tab": tab,
        "classes": relevant_classes,            # queryset of ClassRoom
        "subjects": assigned_subjects,          # queryset of AssignSubject
        ...
        "class_teacher_ids": class_teacher_ids,  # ✅ used in template JS
        "role": logged_in_user.role,
...
from django.utils import timezone
from .models import Attendance, AssignSubject, user, ClassRoom
...
def submit_attendance(request):
    ...
    if not class_id or not assign_subject_id:
        return redirect('teacher')  # invalid request
    ...
    is_class_teacher = cls.class_teacher == logged_in_user
    is_subject_teacher = assign_obj.teacher == logged_in_user

    # Only allow marking if:
    # - Subject teacher for this subject OR
    # - Class teacher and the subject has a teacher assigned
    if not (is_subject_teacher or (is_class_teacher and assign_obj.teacher)):
        return redirect('teacher')  # not allowed
    ...
    # 6️⃣ Get students in the class
    students_in_class = user.objects.filter(role='student', class_name=cls)

    # 7️⃣ Delete existing attendance for this class + subject + date
    Attendance.objects.filter(subject=assign_obj, date=attendance_date).delete()

    # 8️⃣ Save attendance
    for student in students_in_class:
        field_name = f'present_{student.id}'
        status = "Present" if request.POST.get(field_name) in ("on", "present", "1") else "Absent"
    ...
    # 9️⃣ Redirect back to teacher attendance tab
