...
def teacher_view_students(request):
    ...
    students_list = []   # final list to send to template
    added_ids = set()    # to avoid duplicates

    # 1️⃣ CLASS TEACHER → All students of that class
    classroom = ClassRoom.objects.filter(class_teacher=teacher).first()

    if classroom:
        class_students = Student.objects.filter(class_name=classroom)
        for s in class_students:
            students_list.append({
                "student": s,
    ...
    # 2️⃣ SUBJECT TEACHER → Classes assigned through AssignSubject
    subject_classes = AssignSubject.objects.filter(teacher=teacher).values_list(
        "assigned_class", flat=True
    )
