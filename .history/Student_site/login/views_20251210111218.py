...
def teacher_view_students(request):
    ...
    subject_teacher_students = []

    added_ids = set()  # avoid duplicates

    # 1️⃣ CLASS TEACHER STUDENTS
    if class_teacher_class:
        qs = user.objects.filter(role="student", class_name=class_teacher_class)
        for stu in qs:
            class_teacher_students.append(stu)
            added_ids.add(stu.id)

    # 2️⃣ SUBJECT TEACHER STUDENTS
    assigned_classes = AssignSubject.objects.filter(
        teacher=logged_user
    ).values_list("assigned_class", flat=True)

    subject_qs = user.objects.filter(role="student", class_name__in=assigned_classes)

    # Apply filter if selected
    if filter_class:
        subject_qs = subject_qs.filter(class_name_id=filter_class)

    for stu in subject_qs:
        if stu.id not in added_ids:  # no duplicates
            subject_teacher_students.append(stu)
            added_ids.add(stu.id)

    # For dropdown filter
    all_classes = ClassRoom.objects.filter(id__in=assigned_classes)
