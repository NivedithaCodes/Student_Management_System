...
def view_students(request):
    ...
    # FILTER PARAM
    filter_class = request.GET.get("filter_class")   # <--- FIXED NAME
    ...
    # --------------- FIXED BLOCK FOR DROPDOWN -----------------
    all_classes = ClassRoom.objects.filter(
        Q(id__in=subject_class_ids) |
        Q(class_teacher=logged_user)
    ).distinct()
    # ------------------------------------------------------------

    # CLASS TEACHER STUDENTS
    class_teacher_students = user.objects.none()
    if class_teacher_class:
        class_teacher_students = user.objects.filter(
            role="student", class_name=class_teacher_class
        )
