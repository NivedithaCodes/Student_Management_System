...
def view_students(request):
    ...
    filter_class = request.GET.get("class")  # None if not filtering

    # CLASS TEACHER CLASS
    class_teacher_class = ClassRoom.objects.filter(class_teacher=logged_user).first()

    # SUBJECT TEACHER CLASS IDs
    subject_class_ids = AssignSubject.objects.filter(teacher=logged_user).values_list("assigned_class", flat=True)

    # ALL CLASSES for dropdown
    all_classes = ClassRoom.objects.filter(id__in=subject_class_ids)
    if class_teacher_class:
        all_classes = all_classes | ClassRoom.objects.filter(id=class_teacher_class.id)
    all_classes = all_classes.distinct()

    # CLASS TEACHER STUDENTS
    class_teacher_students = []
    if class_teacher_class:
        class_teacher_students = user.objects.filter(role="student", class_name=class_teacher_class)

    # SUBJECT TEACHER STUDENTS
    subject_teacher_students = user.objects.filter(role="student", class_name_id__in=subject_class_ids)
    if class_teacher_class:
        subject_teacher_students = subject_teacher_students.exclude(class_name=class_teacher_class)
    ...
    # Render same dashboard template as before (TeacherPages/teacher.html)
    return render(request, "TeacherPages/teacher.html", context)
