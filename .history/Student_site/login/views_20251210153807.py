...
#     return render(request, "TeacherPages/teacher.html", context)
 ...
 def view_students(request):
    ...
    # ⭐ STUDENTS LIST
    if filter_class:
        # If filter is applied, show only selected class
        class_teacher_students = user.objects.filter(
            role="student",
            class_name_id=filter_class
        ).filter(class_name=class_teacher_class) if class_teacher_class and int(filter_class) == class_teacher_class.id else user.objects.none()

        subject_teacher_students = user.objects.filter(
            role="student",
            class_name_id=filter_class
        ).exclude(class_name=class_teacher_class) if class_teacher_class else user.objects.filter(role="student", class_name_id=filter_class)
    else:
        # If no filter applied, show all students for all classes relevant to this teacher
        class_teacher_students = user.objects.filter(
            role="student",
            class_name=class_teacher_class
        ) if class_teacher_class else user.objects.none()
        ...
        # remove duplicates if class teacher students are included
        if class_teacher_class:
