...
def teacher_view(request):
    ...
    # Determine default_class_id for side panel link
    if class_teacher_class:
        default_class_id = class_teacher_class.id
    elif subject_classes.exists():
        default_class_id = subject_classes.first().id
    else:
        default_class_id = "" 
    context = {
        "tab": tab,
...
#            Q(id__in=subject_class_ids) | Q(id=class_teacher_class.id)
#         ).distinct()
#     else:
#         all_classes = ClassRoom.objects.filter(id__in=subject_class_ids).distinct()
...
#     return render(request, "TeacherPages/teacher_students.html", {
...
def view_students(request):
    ...
    filter_class = request.GET.get("class")  # Class selected in dropdown
    ...
    # Combine all classes for dropdown
    all_classes = ClassRoom.objects.filter(
        Q(id__in=subject_class_ids) | Q(id=class_teacher_class.id if class_teacher_class else 0)
    ).distinct()

    # If no filter selected, set default class to first class teacher class or first subject class
    if not filter_class:
        if class_teacher_class:
            filter_class = str(class_teacher_class.id)
        elif subject_class_ids:
            filter_class = str(subject_class_ids[0])
        else:
            filter_class = None

    # Students in class teacher class
    class_teacher_students = user.objects.filter(
        role="student",
        class_name_id=filter_class
    )
    # Exclude class teacher class from subject_teacher_students to avoid duplicates
    subject_teacher_students = user.objects.filter(
        role="student",
        class_name_id__in=subject_class_ids
    )
    if class_teacher_class:
        subject_teacher_students = subject_teacher_students.exclude(class_name_id=class_teacher_class.id)
