...
def submit_attendance(request):
    ...
    Attendance.objects.filter(student__class_name=cls, subject=assign_obj, date=attendance_date).delete()

    # Save new attendance
    students_in_class = user.objects.filter(role='student', class_name=cls)
    for student in students_in_class:
        present_field = f"present_{student.id}"
        status = "Present" if request.POST.get(present_field) else "Absent"
        Attendance.objects.create(student=student, subject=assign_obj, date=attendance_date, status=status)

    # ✅ Redirect to View Attendance page with class, subject, date pre-filled
    return redirect(f"{reverse('view_attendance')}?class_id={cls.id}&subject_id={assign_obj.id}&date={attendance_date}")
