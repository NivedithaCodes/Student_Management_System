...
def submit_attendance(request):
    ...
    # Map
    attendance_map = {
    att.student.admission_no: att.status.strip().lower()
    ...
    for stu in students_in_class:
     students_with_attendance.append({
        ...
        "status": attendance_map.get(stu.admission_no, "absent")
    })
...
from django.utils import timezone
from datetime import datetime
from login.models import Attendance, ClassRoom, AssignSubject, user
...
def view_attendance(request):
    ...
    classes = ClassRoom.objects.filter(
        id__in=AssignSubject.objects.filter(
            teacher=logged_in_user
        ).values_list("assigned_class", flat=True)
    ).distinct()

    subjects = AssignSubject.objects.filter(teacher=logged_in_user)
    today = timezone.localdate().strftime("%Y-%m-%d")
    ...
    if not (class_id and subject_id):
     return render(request, "TeacherPages/view_attendance.html", {
        "classes": ClassRoom.objects.filter(
            id__in=AssignSubject.objects.filter(
                teacher=logged_in_user
            ).values_list("assigned_class", flat=True)
    ...
    for stu in students_in_class:
        status = attendance_map.get(stu.admission_no)
        if status == "present":
            total_present += 1
        elif status == "absent":
            total_absent += 1
