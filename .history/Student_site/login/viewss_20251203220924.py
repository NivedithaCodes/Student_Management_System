...
def add_class(request):
    if request.method == "POST":
        ...
        cls = ClassRoom(class_name=class_name, section=section)
        if teacher_id:
            cls.class_teacher = user.objects.get(id=teacher_id)
        cls.save()
        messages.success(request, "Class added successfully!")
    return redirect("/principal/?tab=classes")
...
def add_subject(request):
    ...
    return redirect("/principal/?tab=subjects")
...
def delete_timetable(request, id):
      ...
      <a onclick="showPage('settingsPage')">Settings</a>
       ...
       <table>
          ...
          <tr>
            ...
            <td>
              {% if sub.teacher %}{{ sub.teacher.name }}{% else %}-{% endif %}
            </td>
            <td>
 ...
 {% for a in assigned_subjects %}
          <tr>
            ...
            <td>{{ a.subject.subject_name }} ({{ a.subject.subject_code }})</td>

            <td>
              {{ a.assigned_class.class_name }} {{ a.assigned_class.section }}
            </td>

            <td>
              {% if a.teacher %} {{ a.teacher.name }} {% else %} - {% endif %}
            </td>

            <td>
              <a href="{% url 'edit_subject' a.id %}">Edit</a>
              <a href="{% url 'delete_subject' a.id %}">Delete</a>
            </td>
          </tr>
          {% endfor %}